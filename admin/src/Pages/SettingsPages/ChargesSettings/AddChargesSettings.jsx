...
const AddChargesSettings = () => {
  ...
  const [shipCharge, setShipCharge] = useState(
    existingCharges.Normal_Ship_Charge || 0
  );

  const [couponDiscount, setCouponDiscount] = useState(
    existingCharges.Normal_Coup_Disc || 0
  );

  const [coins_reward_user, setCoins_reward_user] = useState(
    existingCharges.coins_reward_user || 0
  );

  const [usage_limit_user, setUsage_limit_user] = useState(
    existingCharges.usage_limit_user || 0
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch(
        `${url}/charges/update/${existingCharges?._id}`,
        charges,
        {
          headers: {
            Authorization: `${adminToken}`,
