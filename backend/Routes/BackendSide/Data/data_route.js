...
// Create Data
route.post('/add', checkAdminOrRole2, async (req, res) => {
    try {
...
//                 data_Image: `${process.env.IP_ADDRESS}/${data.Data_Image?.path?.replace(/\\/g, '/')}` || "",
...
// Update data
route.patch('/update/:id', checkAdminOrRole2, async (req, res) => {
    try {
        const dataId = req.params.id;
